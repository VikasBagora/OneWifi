...
int rdkb_dbus_reg_obj_per_iface(char *path, rdkb_dbus_wifi_obj_desc_t *obj_desc)
{   
        DBusConnection *con;
        DBusError error;
        ...
        if (!dbus_connection_try_register_object_path(con, path, &vtable,
                ...
                if (strcmp(error.name, DBUS_ERROR_OBJECT_PATH_IN_USE) == 0) {
                        ...
                        printf("dbus: Could not set up message handler for object %s (error: %s message: %s)",
        ...
        DBusMessageIter *iter, rdkb_dbus_dict_entry_t *entry)
{
        ...
        buffer = calloc(BYTE_ARRAY_CHUNK_SIZE, BYTE_ARRAY_ITEM_SIZE);
        if (!buffer) {
                return FALSE;
	}
        ...
        rdkb_dbus_dict_entry_t *entry)
{
        ...
        buffer = calloc(STR_ARRAY_CHUNK_SIZE, STR_ARRAY_ITEM_SIZE);
        if (buffer == NULL)
                return FALSE;
                                    ...
                                    rdkb_dbus_dict_entry_t * entry)
{
        ...
        printf( "%s: dict entry key: %s\r\n", __func__, key);
        entry->key = key;

        if (!dbus_message_iter_next(&iter_dict_entry)) {
                printf( "%s: no variant in dict entry\r\n", __func__);
                goto error;
        }
