    ...
    rdk_wifi_vap_info_t *rdk_config)
{
    ...
    wifi_util_dbg_print(WIFI_DB,"%s:%d: vap_array_index:%d vap_index:%d vap_name:%s, band:%d\n",__func__, __LINE__, vap_array_index, vap_index,
                                        wifi_hal_cap_obj->wifi_prop.interface_map[vap_array_index].vap_name, band);
	...
	if (g_wifidb->ctrl.network_mode != rdk_dev_mode_type_sta) {
        ...
        wifi_util_dbg_print(WIFI_DB,"%s:%d: vap_array_index %d vap_index %d vap_name %s, ssid:%s, key:%s\n",
            __func__, __LINE__, vap_array_index, vap_index, wifi_hal_cap_obj->wifi_prop.interface_map[vap_array_index].vap_name,
	    cfg.u.sta_info.ssid, cfg.u.sta_info.security.u.key.key);
        ...
        if ((strlen(cfg.u.sta_info.security.u.key.key) < MIN_PWD_LEN) || (strlen(cfg.u.sta_info.security.u.key.key) > MAX_PWD_LEN)) {
            wifi_util_error_print(WIFI_DB, "%s:%d: Incorrect password length %d for vap '%s'\n", __func__, __LINE__,
                strlen(cfg.u.sta_info.security.u.key.key), vap_name);
